...
public class OnekeyShare {
	...
	public void HideCustomerLogo(int index)
	{
		ArrayList<CustomerLogo> customers = ResHelper.forceCast(params.get("customers"));
	...
	public void show(Context context) {
       ...
       ArrayList<CustomerLogo> customerLogos= themeImpl.customerLogos;
		if(customerLogos!=null && customerLogos.size()>0)
		{
			try {
				for (int i = 0; i < customerLogos.size() - 1; i++) {
					for (int j = customerLogos.size() - 1; j > i; j--) {
						if (customerLogos.get(j).label.equals(customerLogos.get(i).label)) {
							customerLogos.remove(j);
						}
					}
				}
				themeImpl.setCustomerLogos(customerLogos);
			}catch(Throwable t){}
		}
