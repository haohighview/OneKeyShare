...
public abstract class OnekeyShareThemeImpl implements PlatformActionListener, Callback {
	...
	final boolean formateShareData(Platform plat) {
		...
		boolean isLinkedIn = "LinkedIn".equals(name);
		if (isLinkedIn && !plat.isClientValid()) {
			toast("ssdk_linkedin_client_inavailable");
			return false;
		}
		...
		boolean isQQ= "QZone".equals(name) || "QQ".equals(name);
		if ( isQQ && !plat.isClientValid()) {
